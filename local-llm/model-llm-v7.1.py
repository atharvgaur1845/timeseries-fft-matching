...
class MultiScaleCNNExtractor(nn.Module):
    ...
    def __init__(self, input_channels, output_channels):
        ...
        self.conv_2 = CNNEncoderBlock(input_channels, output_channels//4, kernel_size=3, dilation=2)
        self.conv_4 = CNNEncoderBlock(input_channels, output_channels//4, kernel_size=3, dilation=4)
        self.conv_8 = CNNEncoderBlock(input_channels, output_channels//4, kernel_size=3, dilation=8)
        self.fusion = nn.Conv1d(output_channels, output_channels, kernel_size=1)
        self.fusion_bn = nn.BatchNorm1d(output_channels)
        self.fusion_act = nn.GELU()
        
    def forward(self, x):
        feat_1 = self.conv_1(x)
        feat_2 = self.conv_2(x)
