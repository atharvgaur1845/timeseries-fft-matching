...
class SelfAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        # Attention weights - scaled dot-product attention
        attention = torch.bmm(Q, K) / math.sqrt(self.in_channels // self.reduction)  # (B, L, L)
...
class GeneratorTCNWithSelfAttention(nn.Module):
    def __init__(self, nz=100, num_classes=11, embed_size=10, num_blocks=9, 
        ...
        # Initial processing to create starting sequence 
...
Generator = GeneratorTCNWithSelfAttention
...
def plot_ffts_all(original_freqs, original_mags, generated_freqs, generated_mags, titles):
    ...
    for i in range(min(10, len(titles))):  # Updated to handle variable number of titles
        plt.subplot(5, 2, i + 1)
...
percentage = 10
num_train_samples = (4800 * percentage) // 100
weight_path = f'./{num_train_samples}_wgan-True_GeneratorTCNWithSelfAttention-DiscriminatorTCNG_50.pth'  # Updated filename
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

GeneratorModel = GeneratorTCNWithSelfAttention  # Updated to use new GeneratorTCNWithSelfAttention
